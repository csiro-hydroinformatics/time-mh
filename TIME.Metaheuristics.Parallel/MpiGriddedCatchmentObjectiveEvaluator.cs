...
namespace TIME.Metaheuristics.Parallel
{
    ...
    public sealed class MpiGriddedCatchmentObjectiveEvaluator : IEnsembleObjectiveEvaluator<MpiSysConfig>, IDisposable
    {
        ...
        /// </summary>
        private readonly Dictionary<string, Intracommunicator> communicatorsByCatchmentId =
            new Dictionary<string, Intracommunicator>();
        ...
        {
            WorldRank = Communicator.world.Rank;
            IsFirstRun = true;

            if (IsMaster && Communicator.world.Size < 2)
        ...
        private WorkPackage MyWork
        {
            ...
            set
            {
                ...
                if (myWork != null && myWork.Cells.Length > 0)
                {
                    ...
                    {
                        Log.DebugFormat("Rank {0}: model instance {1}", WorldRank, i);
        ...
        private void AllocateWork(IWorkAllocator workAllocator)
        {
            ...
            if (IsMaster && TotalCellCount < Communicator.world.Size - 1)
        ...
        private void CreateCommunicators(IWorkAllocator workAllocator)
        {
            ...
            foreach (CatchmentDefinition catchment in GlobalDefinition)
            {
                ...
                Group catchmentGroup = Communicator.world.Group.IncludeOnly(workAllocator.RanksByCatchment[catchment.Id].ToArray());
                Log.DebugFormat("Rank {0}: Catchment group created, rank = {1} size = {2}", WorldRank, catchmentGroup.Rank, catchmentGroup.Size);
                Intracommunicator catchmentCommunicator = (Intracommunicator)Communicator.world.Create(catchmentGroup);
                if (catchmentCommunicator != null)
                    Log.DebugFormat("Rank {0}: Communicator created, rank = {1} size = {2}", WorldRank, catchmentCommunicator.Rank, catchmentCommunicator.Size);
                else
                ...
                if (catchmentCommunicator != null)
                {
                    ...
                    if (catchmentCommunicator.Rank == 0)
                    {
        ...
        public void Execute(MpiSysConfig parameters)
        {
            ...
            catchmentScores = Communicator.world.GatherFlattened(new MpiObjectiveScores[0], NumCatchmentResultsPerWorker, 0);
        ...
        private void DoWork(MpiSysConfig parameters)
        {
            ...
            Communicator.world.GatherFlattened(finalCatchmentResults, 0);
            ...
            Dictionary<string, SerializableDictionary<string, MpiTimeSeries>> partialCatchmentResults, MpiSysConfig sysConfig)
        {
            ...
            foreach (CatchmentDefinition catchment in MyWork.Catchments)
            {
                ...
                if (catchmentComm.Rank == 0) //Catchment coordinator
                {
                    SerializableDictionary<string, MpiTimeSeries>[] completeCatchmentResults = catchmentComm.Gather(partialCatchmentResults[catchment.Id], 0);
                ...
                else
                {
                    ...
                    catchmentComm.Gather(partialCatchmentResults[catchment.Id], 0);
                }
            }
